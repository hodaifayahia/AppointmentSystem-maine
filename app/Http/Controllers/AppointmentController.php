...
class AppointmentController extends Controller
{
    ...
    public function consulationappointment(Request $request, $doctorId)
    {
        try {
            ...
            if ($request->has('statuses') && is_array($request->statuses)) {
                ...
                $query->where(function ($q) use ($statuses) {
                    foreach ($statuses as $status) {
                        $q->orWhere(function ($subQuery) use ($status) {
                            ...
                            // ONWORKING (5) and DONE appointments should show regardless of date
                            if ($status != 5 && $status != AppointmentSatatusEnum::DONE->value) {
                                // This is the modification: Filter for today's appointments
                ...
                if ($request->status != AppointmentSatatusEnum::CANCELED->value) {
                    // Apply 'today' filter only if status is not ONWORKING (5) and not DONE
                    if ($request->status != 5 && $request->status != AppointmentSatatusEnum::DONE->value) {
                        // This is the modification: Use a different filter for 'today'
    ...
    public function ForceAppointment(Request $request)
    {
        ...
        try {
            ...
            $schedules = Schedule::where('doctor_id', $doctorId)
                ...
                ->where(function ($query) use ($date, $dayOfWeek) {
                    $query->where('date', $date) // Specific date schedule
                        ->orWhereNull('date') // Recurring schedule
                        ->where('day_of_week', $dayOfWeek);
                })
                ->get();

            // Check for complete exclusion
            $isCompletelyExcluded = ExcludedDate::where('doctor_id', $doctorId)
            ...
            if ($isCompletelyExcluded) {
                return response()->json([
                    'message' => 'The doctor is completely excluded on this date.',
                    'gap_slots' => [],
                    'additional_slots' => [],
                    'next_available_date' => $date,
                    'time_slot_minutes' => null,
                ]);
            }

            // Determine time slot minutes based on doctor's setting or calculated from schedules
            ...
            // If there are no regular schedules, but the date is "limited" excluded, or no schedule at all,
            // generate slots based on AppointmentForcer or default 8:00-17:00
            $isLimitedExcluded = ExcludedDate::where('doctor_id', $doctorId)
                ->where('exclusionType', 'limited')
                ->where(function ($query) use ($date) {
                    $query->where('start_date', '<=', $date)
                        ->where(function ($q) use ($date) {
            ...
            if ($schedules->isEmpty() || $isLimitedExcluded) {
    ...
    private function generateDefaultSlots($doctorId, $date, $timeSlotMinutes)
    {
        ...
        $bookedSlots = $this->getBookedSlots($doctorId, $date); // Get all booked slots for the day
     ...
     */
    private function handleSchedules($morningSchedule, $afternoonSchedule, $date, $timeSlotMinutes, $doctorId)
    {
        $gapSlots = [];
        $additionalSlots = [];
        ...
        if ($morningSchedule && !$afternoonSchedule) {
            ...
            $additionalSlots = $this->generateSlotsAfterTime($morningEndTime, $timeSlotMinutes, $doctorId, $date);
        } elseif (!$morningSchedule && $afternoonSchedule) {
            $afternoonEndTime = Carbon::parse($date . ' ' . $afternoonSchedule->end_time);
            $additionalSlots = $this->generateSlotsAfterTime($afternoonEndTime, $timeSlotMinutes, $doctorId, $date);
        } elseif ($morningSchedule && $afternoonSchedule) {
            $morningEndTime = Carbon::parse($date . ' ' . $morningSchedule->end_time);
     ...
     */
    private function generateSlotsAfterTime($endTime, $timeSlotMinutes, $doctorId, $date)
    {
        $slots = [];
        $currentTime = clone $endTime;
        $bookedSlots = $this->getBookedSlots($doctorId, $date);

        for ($i = 0; $i < 20; $i++) { // Limiting to 20 additional slots for practicality
            $currentTime->addMinutes($timeSlotMinutes);
            $slotTime = $currentTime->format('H:i');
            if (!in_array($slotTime, $bookedSlots)) {
                $slots[] = $slotTime;
    ...
    public function changeAppointmentStatus(Request $request, $id)
    {
        ...
        // Set consultation_end_at when status is Done (4)
        if ($validated['status'] == 4) {
            $consultation = \App\Models\Consultation::where('appointment_id', $appointment->id)->first();
            if ($consultation) {
                $consultation->consultation_end_at = now();
                $consultation->save();
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (
            !Appointment::isSlotAvailableForUpdate(
                $doctorId,
                $appointmentDate,
                $appointmentTime,
                $this->excludedStatuses,
